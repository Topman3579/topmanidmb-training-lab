  ...
  return (
    <div className="space-y-8 animate-fade-in-up">
      ...
      <section className="lab-card p-6 sm:p-8">
        <SectionTitle
          kicker="Dashboard Zones"
          title="6 พื้นที่หลักของ War Room Dashboard"
          desc="ใช้เป็นโครงหน้าแรกสำหรับ Command Center ที่ต้องสรุปสถานการณ์ได้ในจอเดียว"
        />
        ...
        <div className="lab-card p-6 sm:p-8">
          ...
          <div className="space-y-3">
            {layoutRules.map((item) => (
              <div key={item.label} className="flex gap-3 rounded-xl border border-navy-900/10 bg-ivory-50 p-4 text-sm text-navy-700">
                <span className="min-w-14 rounded-lg bg-navy-900 px-2 py-1 text-center text-xs font-bold text-ivory-50">{item.label}</span>
      ...
      <section className="lab-card border-gold-400/40 bg-gradient-to-br from-brand-50 to-paper p-6 sm:p-8">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {guardrails.map((rule) => (
            <div key={rule} className="flex gap-3 rounded-xl border border-navy-900/10 bg-paper p-3 text-sm text-navy-700">
      ...
      <section className="lab-card p-6 sm:p-8">
        <SectionTitle kicker="Training Plan" title="แผนฝึก 7 วัน" desc="ฝึกจากโครง dashboard ง่าย ๆ ไปสู่ executive war room ที่ใช้ได้จริง" />
